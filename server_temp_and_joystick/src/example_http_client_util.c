...
// Imprime cabeçalhos na saída padrão (stdout)
...
// Imprime corpo da resposta na saída padrão (stdout)
err_t http_client_receive_print_fn(__unused void *arg, __unused struct altcp_pcb *conn, struct pbuf *p, err_t err)
{
    ...
    // IMPORTANTE: liberar o buffer após o uso
    pbuf_free(p);
...
static void internal_result_fn(void *arg, httpc_result_t httpc_result, u32_t rx_content_len, u32_t srv_res, err_t err)
{
    assert(arg);
    EXAMPLE_HTTP_REQUEST_T *req = (EXAMPLE_HTTP_REQUEST_T *)arg;
    HTTP_DEBUG("resultado %d tamanho %u resposta_servidor %u err %d\n", httpc_result, rx_content_len, srv_res, err);
    req->complete = true;
...
// Sobrescreve altcp_tls_alloc para configurar SNI (Server Name Indication)
static struct altcp_pcb *altcp_tls_alloc_sni(void *arg, u8_t ip_type)
{
    ...
    if (!pcb)
    {
        HTTP_ERROR("Falha ao alocar PCB\n");
        return NULL;
    }
...
// Faz uma requisição HTTP, completa quando req->complete retorna true
int http_client_request_async(async_context_t *context, EXAMPLE_HTTP_REQUEST_T *req)
{
    ...
    if (ret != ERR_OK)
    {
        HTTP_ERROR("requisição HTTP falhou: %d", ret);
    }
    return ret;
}

// Faz uma requisição HTTP e só retorna quando estiver completa. Retorna true em caso de sucesso
